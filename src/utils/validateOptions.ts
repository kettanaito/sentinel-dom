...
export default function validateOptions(options: Object, rules: Object): void {
  Object.keys(rules).forEach((propName) => {
    ...
    if (propRule.required) {
      if (!propValue)
        throw new Error(
          `Property "${propName}" is required, but was not found in the provided properties.`,
        )
    }

    if (propValue) {
      const hasCustomRule = typeof propRule.expect === 'function'
      const isValid = hasCustomRule
        ? propRule.expect(propValue)
        : typeof propValue === propRule.expect
      ...
      if (!isValid) {
        ...
        const errorMessage = propRule.message
          ...
          : `Expected property "${propName}" to be an instance of type ${
              propRule.expect
            }, but got: ${receivedProp}.`
